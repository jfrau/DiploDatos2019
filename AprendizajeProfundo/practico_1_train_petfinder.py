...
def main():
    ...
    train_ds = tf.data.Dataset.from_tensor_slices((X_train['direct_features'], y_train)).batch(batch_size)
    dev_ds = tf.data.Dataset.from_tensor_slices((X_dev['direct_features'], y_dev)).batch(batch_size)
    test_ds = tf.data.Dataset.from_tensor_slices(X_test['direct_features']).batch(batch_size)
    
    # TODO: Build the Keras model
    
    model = Sequential([
        Dense(10, input_shape=direct_features_input_shape, activation='relu',kernel_regularizer=regularizers.l2(0.001)),
        Dropout(0.3),
